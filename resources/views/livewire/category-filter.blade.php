<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
        <aside>
            ...
            <h2 class="font-semibold mb-5 mt-4">Marcas</h2>
            <ul class="divide-y divide-gray-200">
                @foreach ($category->brands as $brand)
                    <li class="py-2 text-sm">
                        <a 
                        wire:click = "$set('marca','{{$brand->name}}')" class="cursor-pointer hover:text-orange-500 capitalize {{$brand->name == $marca ? 'text-orange-500 font-semibold' : ' '}}">{{$brand->name}}</a>
                    </li>
                @endforeach
            </ul>
